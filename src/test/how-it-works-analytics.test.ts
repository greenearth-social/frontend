...
describe("HowItWorks analytics", () => {
  ...
  it("captures opening a diagram component but not closing it", async () => {
    ...
    const component = element.shadowRoot?.querySelector<HTMLElement>(
      ".config-pill",
    );
    component?.click();
    await element.updateComplete;
  ...
  it("captures every supported diagram component ID", async () => {
    ...
    const components = [
      ...
      ...Array.from(
        element.shadowRoot?.querySelectorAll<HTMLElement>(".node-box-source") ?? [],
      ),
      ...Array.from(
        element.shadowRoot?.querySelectorAll<HTMLElement>(".node-box-signal") ?? [],
      ),
      element.shadowRoot?.querySelector<HTMLElement>(".engaging-pill"),
      ...Array.from(
        element.shadowRoot?.querySelectorAll<HTMLElement>(".penalty-pill") ?? [],
      ),
    ];
